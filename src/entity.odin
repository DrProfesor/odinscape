...
em_load_scene :: proc(folder_path: string) {
	assert(scene == nil);
...
em_save_scene :: proc() {
	...
	for eid, data in scene.entity_datas {
		...
		for c in data.components {
			...
			_comp, found :=_em_get_component_internal(eid, c);
			assert(found);
...
em_add_component_type :: proc($Type: typeid, update_proc: proc(^Type, f32), render_proc: proc(^Type), init_proc: proc(^Type) = nil) {
	...
	component_types[id] = new_clone(Component_Type{type_info_of(Type), transmute(mem.Raw_Dynamic_Array)make([dynamic]Type, 0, 1), make([dynamic]int, 0, 1), cast(proc(rawptr, f32))update_proc, cast(proc(rawptr))render_proc, cast(proc(rawptr))init_proc});
}
...
em_make_entity :: proc(name := "Entity", requested_id: Entity = 0) -> Entity {
	...
	data^ = Entity_Data{name, component_list, ""};
...
em_destroy_entity_immediate :: proc(eid: Entity) {
	assert(scene != nil);
...
em_add_component :: proc(eid: Entity, $T: typeid, loc := #caller_location) -> ^T { // note(josh): volatile return value, do not store
	ptr := _em_add_component_internal(eid, typeid_of(T), loc);
	assert(ptr != nil);
	return cast(^T)ptr;
}

em_get_component :: proc(eid: Entity, $T: typeid, loc := #caller_location) -> (^T, bool) {
	ptr, ok := _em_get_component_internal(eid, typeid_of(T));
...
em_update :: proc(dt: f32) {
	...
	// destroy entities
	{
		for eid in scene.entities_to_destroy {
			em_destroy_entity_immediate(eid);
		}
...
_em_add_component_internal :: proc(eid: Entity, tid: typeid, loc := #caller_location) -> ^Component_Base {
	assert(scene != nil);
	assert(eid != 0);
    
	ti := type_info_of(tid);
    
	if _, already_exists := _em_get_component_internal(eid, tid); already_exists {
...
_em_get_component_internal :: proc(eid: Entity, tid: typeid) -> (^Component_Base, bool) {
...
_em_copy_component_internal :: proc(dst: ^Component_Base, src: ^Component_Base, ti: ^rt.Type_Info) {
	tmp := dst^;
...
load_entity_from_file :: proc(filepath: string) -> Entity {
	...
	// make it
	em_make_entity(entity_name, eid);
	...
	for {
		...
		comp, found := _em_get_component_internal(eid, ti.id);
		if !found do comp = _em_add_component_internal(eid, ti.id);
...
draw_entity_window :: proc() {
	if imgui.begin("Scene") {
		// save/load panel
		{
			@static _scene_to_load: [1024]u8;
			imgui.input_text("Load scene", cast([]u8)_scene_to_load[:]);
			_scene_to_load[len(_scene_to_load)-1] = 0;
			scene_to_load := cast(string)cast(cstring)&_scene_to_load[0];
			...
			// load / save
			{
				...
				if asking_to_save {
					...
					if do_load_after_save {
						...
						scene_folder := SSTRING("scenes/", cast(string)cast(cstring)&_scene_to_load[0]);
						em_load_scene(scene_folder);
						_scene_to_load = {};
					}
				}
						...
						for tid, data in component_types {
							...
							if len(input_lower) > 0 && string_starts_with(name_lower, input_lower) {
								if imgui.button(name) {
									comp := cast(^Component_Base)_em_add_component_internal(eid, tid);
									imgui.close_current_popup();
								}
							}
						}
				...
				if !ok {
					...
					for c in to_clone_entity_data.components {
						...
						if c == type_info_of(Transform).id {
							comp, ok = _em_get_component_internal(new_entity, c);
							assert(ok);
						}
						else {
							comp = _em_add_component_internal(new_entity, c);
						}
						to_clone_component, ok := _em_get_component_internal(entity_to_clone, c);
						assert(ok);
						_em_copy_component_internal(comp, to_clone_component, type_info_of(c));
					}
				}
			}
		}
...
@(deferred_out=delete_sstring)
SSTRING :: proc(args: ..any) -> string {
	return aprint(..args);
}

delete_sstring :: proc(str: string) {
	delete(str);
}